...
class E2eKeysHandlerTestCase(unittest.HomeserverTestCase):
    ...
    # PG_CHANGED
    def test_query_devices_hides_other_user_display_names(self) -> None:
        """Test that device display names are hidden when querying other users' devices,
        but shown when querying own devices.
        
        PG_CHANGED: This test verifies the privacy feature that prevents users from
        seeing other users' device display names via /keys/query.
        """
        # Set up two users with devices
        requesting_user = "@alice:" + self.hs.hostname
        other_user = "@bob:" + self.hs.hostname
        
        requesting_device_id = "ALICEDEVICE"
        other_device_id = "BOBDEVICE"
        
        # Create devices with display names
        ...
        self.get_success(
            device_handler.check_device_registered(
                ...
                initial_device_display_name="Bob's Laptop",
            )
        )
        
        # Upload device keys for both users
        requesting_user_device_key: JsonDict = {
            "user_id": requesting_user,
            "device_id": requesting_device_id,
            "algorithms": [
            ...
            "keys": {
                "curve25519:" + requesting_device_id: "curve25519+alice+key",
                "ed25519:" + requesting_device_id: "ed25519+alice+key",
            },
            "signatures": {requesting_user: {"ed25519:" + requesting_device_id: "sig"}},
        }
        
        other_user_device_key: JsonDict = {
            "user_id": other_user,
            "device_id": other_device_id,
            "algorithms": [
                "m.olm.curve25519-aes-sha2",
            ...
            "keys": {
                "curve25519:" + other_device_id: "curve25519+bob+key",
                "ed25519:" + other_device_id: "ed25519+bob+key",
            },
            "signatures": {other_user: {"ed25519:" + other_device_id: "sig"}},
        }
        
        self.get_success(
            self.handler.upload_keys_for_user(
                requesting_user, requesting_device_id, {"device_keys": requesting_user_device_key}
            )
        )
        self.get_success(
            self.handler.upload_keys_for_user(
                other_user, other_device_id, {"device_keys": other_user_device_key}
            )
        )
        
        # Query both users' devices from the perspective of requesting_user
        result = self.get_success(
            self.handler.query_devices(
                {"device_keys": {requesting_user: [], other_user: []}},
                0,
                requesting_user,
        ...
        # Own device should have display name
        self.assertIn(requesting_user, result["device_keys"])
        self.assertIn(requesting_device_id, result["device_keys"][requesting_user])
        own_device = result["device_keys"][requesting_user][requesting_device_id]
        self.assertIn("unsigned", own_device)
        self.assertEqual(
            own_device["unsigned"].get("device_display_name"),
            "Alice's Phone",
            "Own device display name should be visible",
        )
        
        # Other user's device should NOT have display name
        self.assertIn(other_user, result["device_keys"])
        self.assertIn(other_device_id, result["device_keys"][other_user])
        other_device = result["device_keys"][other_user][other_device_id]
        self.assertNotIn(
            "device_display_name",
            other_device.get("unsigned", {}),
            "Other user's device display name should be hidden",
        )
