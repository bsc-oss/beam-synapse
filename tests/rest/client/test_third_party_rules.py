...
class ThirdPartyRulesTestCase(unittest.FederatingHomeserverTestCase):
    ...
    # PG_CHANGED
    def test_on_leave_room_called_on_leave(self) -> None:
        """Test that the on_leave_room callback is called when a user leaves a room."""
        on_leave_room = AsyncMock(return_value=None)
        self.hs.get_module_api_callbacks().third_party_event_rules._on_leave_room_callbacks.append(
            on_leave_room
        )

        # Leave the room.
        self.helper.leave(room=self.room_id, user=self.user_id, tok=self.tok)

        # Check the callback was called once with the right args.
        on_leave_room.assert_called_once()
        args, _ = on_leave_room.call_args
        self.assertEqual(args[0], self.user_id)
        self.assertEqual(args[1], self.room_id)
        # Voluntary leave: requester is the user themselves.
        self.assertEqual(args[2], self.user_id)

    # PG_CHANGED
    def test_on_leave_room_called_on_ban(self) -> None:
        """Test that the on_leave_room callback is called when a user is banned."""
        # First, have invitee join the room so they can be banned.
        invitee_tok = self.login("invitee", "hackme")
        self.helper.invite(
            room=self.room_id, src=self.user_id, targ=self.invitee, tok=self.tok
        )
        self.helper.join(room=self.room_id, user=self.invitee, tok=invitee_tok)
        ...
        # Check the callback was called once with the banned user's ID.
        on_leave_room.assert_called_once()
        args, _ = on_leave_room.call_args
        self.assertEqual(args[0], self.invitee)
        self.assertEqual(args[1], self.room_id)
        # Ban: requester is the user who performed the ban.
        self.assertEqual(args[2], self.user_id)

    # PG_CHANGED
    def test_on_leave_room_called_on_kick(self) -> None:
        """Test that the on_leave_room callback is called when a user is kicked."""
        # First, have invitee join the room so they can be kicked.
        invitee_tok = self.login("invitee", "hackme")
        self.helper.invite(
            room=self.room_id, src=self.user_id, targ=self.invitee, tok=self.tok
        )
        ...
        # Kick the invitee (leave membership set by another user).
        ...
        # Check the callback was called once with the kicked user's ID.
        on_leave_room.assert_called_once()
        args, _ = on_leave_room.call_args
        self.assertEqual(args[0], self.invitee)
        self.assertEqual(args[1], self.room_id)
        # Kick: requester is the user who performed the kick.
        self.assertEqual(args[2], self.user_id)

    # PG_CHANGED
    def test_on_leave_room_not_called_on_join(self) -> None:
        """Test that the on_leave_room callback is NOT called on join or invite."""
        on_leave_room = AsyncMock(return_value=None)
        self.hs.get_module_api_callbacks().third_party_event_rules._on_leave_room_callbacks.append(
            on_leave_room
        )
        ...
        # Neither invite nor join should trigger the callback.
        on_leave_room.assert_not_called()
