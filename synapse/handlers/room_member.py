...
class RoomMemberHandler(metaclass=abc.ABCMeta):
        ...
        *,
        ...
        for i in range(max_retries):
            try:
                ...
                # PG_CHANGED: Fire on_leave_room hook BEFORE persisting the
                # leave/ban, while the user is still considered "in room" for auth.
                if membership in (Membership.LEAVE, Membership.BAN):
                    await self._third_party_event_rules.on_leave_room(
                        user_id,
                        room_id,
    ...
    async def update_membership_locked(
        ...
        # PG_CHANGED: Call the hook to allow modules to enrich the membership content
        await self._third_party_event_rules.on_enrich_membership_content(
            requester.user.to_string(),
            target.to_string(),
            room_id,
            effective_membership_state,
