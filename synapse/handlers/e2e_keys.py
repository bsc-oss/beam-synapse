...
class E2eKeysHandler:
    ...
    async def query_devices(
        ...
        async with self._query_devices_linearizer.queue((from_user_id, from_device_id)):
            ...
            results = {}
            if local_query:
                # PG_CHANGED: Split the query: 
                # - Include display names only for the requesting user's own devices
                # - Hide display name when requesting other user's devices
                own_query = {from_user_id: local_query[from_user_id]} if from_user_id in local_query else {}
                other_query = {uid: devs for uid, devs in local_query.items() if uid != from_user_id}

                # Query own devices with display names
                if own_query:
                    own_result = await self.query_local_devices(own_query, include_displaynames=True)
                    for user_id, keys in own_result.items():
                        results[user_id] = keys

                # Query other local users without display names
                if other_query:
                    other_result = await self.query_local_devices(other_query, include_displaynames=False)
                    for user_id, keys in other_result.items():
                        results[user_id] = keys
