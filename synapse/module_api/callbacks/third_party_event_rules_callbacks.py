...
# PG_CHANGED: Callback for enriching membership event content. Takes (requester_id, user_id, room_id, membership, content)
ON_ENRICH_MEMBERSHIP_CONTENT_CALLBACK = Callable[
    [str, str, str, str, dict], Awaitable
]
# PG_CHANGED: Callback fired just before a leave event is persisted. Takes (user_id, room_id, requester_id)
ON_LEAVE_ROOM_CALLBACK = Callable[[str, str, str], Awaitable]
...
class ThirdPartyEventRulesModuleApiCallbacks:
    ...
    def __init__(self, hs: "HomeServer"):
        ...
        # PG_CHANGED
        self._on_enrich_membership_content_callbacks: list[
            ON_ENRICH_MEMBERSHIP_CONTENT_CALLBACK
        ] = []
        # PG_CHANGED
        self._on_leave_room_callbacks: list[ON_LEAVE_ROOM_CALLBACK] = []
    ...
    def register_third_party_rules_callbacks(
        ...
        | None = None,
        # PG_CHANGED
        on_enrich_membership_content: ON_ENRICH_MEMBERSHIP_CONTENT_CALLBACK
        | None = None,
        # PG_CHANGED
        on_leave_room: ON_LEAVE_ROOM_CALLBACK | None = None,
    ) -> None:
        ...
        # PG_CHANGED
        if on_enrich_membership_content is not None:
            self._on_enrich_membership_content_callbacks.append(
                on_enrich_membership_content
            )

        # PG_CHANGED
        if on_leave_room is not None:
            self._on_leave_room_callbacks.append(on_leave_room)

    # PG_CHANGED 7-unfreeze_notification_levels
    ...
    # PG_CHANGED
    async def on_enrich_membership_content(
        self,
        requester_id: str,
        user_id: str,
        room_id: str,
        ...
        """PG_CHANGED: Called when a membership event is being created, allowing modules to add
        extra content to the event.

        This is called for membership events before the event is created and sent.
        This hook is called for both local and remote (federated) joins.

        The callback receives a mutable content dict and can modify it directly to
        add extra fields (e.g., email, custom profile data).

        Args:
            requester_id: The user ID of who is requesting the membership change (the
                sender of the membership event). This may be different from user_id
                for invites, kicks, and bans.
            user_id: The user ID of the member (state_key of the membership event).
            room_id: The room ID where the membership event is being created.
            membership: The membership type (e.g., "join", "invite", "knock").
            content: The mutable content dict of the membership event. Callbacks
                can modify this dict directly to add extra fields.
        """
        for callback in self._on_enrich_membership_content_callbacks:
            try:
                extra_content = await callback(
                    requester_id, user_id, room_id, membership, content
                )
    ...
    async def on_leave_room(
        ...
        """Called just before a leave event is persisted, while the user is
        still considered "in room" for auth purposes.

        This allows modules to perform actions that require the user to be
        a room member.

        Args:
            user_id: The user ID of the user who is leaving.
            room_id: The room ID the user is leaving.
            requester_id: The user ID of the user requesting the action
                (same as user_id for voluntary leave, different for kick/ban).
        """
        for callback in self._on_leave_room_callbacks:
            try:
                await callback(user_id, room_id, requester_id)
            except Exception as e:
                logger.exception(
                    "Failed to run module API callback %s: %s", callback, e
                )
